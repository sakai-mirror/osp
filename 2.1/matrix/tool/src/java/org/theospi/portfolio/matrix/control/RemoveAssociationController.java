 ...
 * @author chmaurer
 */
public class RemoveAssociationController implements LoadObjectController, CustomCommandController {
   ...
   public ModelAndView handleRequest(Object requestModel, Map request, Map session, Map application, Errors errors) {
      ...
      if (action != null) {
         if (action.equals("Continue")) {
            String strNodeIds = (String) request.get("selectedArtifacts");
            String[] nodeIds = strNodeIds.split(",");
            //Id nodeId = idManager.getId((String)request.get("selectedArtifacts"));
            if (nodeIds != null)
            {
               for (int i=0; i<nodeIds.length; i++)
               {
                  Id nodeId = idManager.getId(nodeIds[i]);
                  matrixManager.detachArtifact(cellId, nodeId);
               }
            }
         }
