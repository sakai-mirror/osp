 ...
 *
 * $Header: /opt/CVS/osp2.x/matrix/tool/src/java/org/theospi/portfolio/matrix/control/MoveExpectationController.java,v 1.1 2005/07/15 21:10:34 rpembry Exp $
 * $Revision$
 * $Date$
 */
 ...
 * @author chmaurer
 */
public class MoveExpectationController implements Controller {
   ...
   public ModelAndView handleRequest(Object requestModel, Map request, Map session, Map application, Errors errors) {
      ...
      if (currentIndex != null && destIndex != null) {
         ...
         int max = scaffoldingCell.getExpectations().size()-1;
         if (dest == -1) dest = max;
         if (dest == max+1) dest = 0;
         //remove then insert
         Expectation obj = (Expectation)scaffoldingCell.getExpectations().remove(current);
         scaffoldingCell.getExpectations().add(dest, obj);
      }
