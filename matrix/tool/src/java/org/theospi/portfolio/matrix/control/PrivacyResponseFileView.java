 ...
 *
 * $Header: /opt/CVS/osp2.x/matrix/tool/src/java/org/theospi/portfolio/matrix/control/PrivacyResponseFileView.java,v 1.4 2005/08/10 21:05:09 chmaurer Exp $
 * $Revision$
 * $Date$
 */
...
public class PrivacyResponseFileView extends XmlElementView {
   ...
   protected String prepareTemplateForRendering(Map model,HttpServletRequest request, HttpServletResponse response) throws Exception {
      PrivacyFormBean bean = (PrivacyFormBean)model.get("bean");

      Scaffolding scaffolding = bean.getCell().getScaffoldingCell().getScaffolding();
      String baseFile="";
      
      Node privacyXsdMetadata = bean.getXsdFile();
      baseFile = getBaseUrl() + "_" + scaffolding.getId().getValue();
      ...
      if (!genFile.exists() ||
            (genFile.lastModified() < privacyXsdMetadata.getTechnicalMetadata().getLastModified().getTime() ||
            genFile.lastModified() < getVelocityTemplate().getLastModified())) {
         return createJspFromTemplate(bean, baseFile + ".jsp", genFile);
      }
   ...
   protected String createJspFromTemplate(PrivacyFormBean bean, String resultFile,
                                          File jspFile) throws Exception {
      VelocityContext context = new VelocityContext();

      SchemaNode schema = bean.getPrivacyResponse().getCurrentSchema();
      Scaffolding scaff = bean.getCell().getScaffoldingCell().getScaffolding();

      context.put("schema",
         new SchemaBean(schema,
            scaff.getDocumentRoot(), null, scaff.getTitle()));
