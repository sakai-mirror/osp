		...
		<tr>
		  ...
		  <h4 style="display:inline">
			<c:if test="${!(empty scaffold.description)}">
				<a href="#"  onclick="toggle_visibility('<c:out value="${scaffold.id.value}" />')"><img  id="toggle<c:out value="${scaffold.id.value}" />"  src="/library/image/sakai/expand.gif" style="padding-top:4px" title="<fmt:message key="hideshowdesc_toggle_show"/>"></a></span>
			</c:if>
			<c:if test="${(empty scaffold.description)}">
		...
		<c:if test="${!(empty scaffold.description)}">
			<tr class="exclude">
					<td colspan="4" style="padding:0">
						<div class="instruction indnt2" id="<c:out value="${scaffold.id.value}" />" style="padding:0;margin:0;display:none">
